...
app.use(flash());
// Remove the busboy related code
...
app.post("/*@upload", (req, res) => {
  ...
  upload(req, res, (err) => {
    ...
    fs.readFile(req.file.path, (err, data) => {
      ...
      fs.writeFile(saveName, data, (err) => {
        if (err) {
          console.error(err);
          req.flash("error", err.toString());
          return res.redirect("back");
...
app.post("/*@mkdir", (req, res) => {
  ...
  if (!folder || folder.trim().length < 1) {
...
app.post("/*@delete", (req, res) => {
  ...
  let promises = files.map((f) => {
    return new Promise((resolve, reject) => {
      let file = relative(res.filename, f);
      fs.stat(file, (err, stats) => {
        if (err) {
          return reject(err);
        }
        resolve({ file, isDirectory: stats.isDirectory() });
      });
    });
  });
  ...
  Promise.all(promises)
    .then((files) => {
      let deletePromises = files.map((f) => {
        return new Promise((resolve, reject) => {
          if (f.isDirectory) {
            rimraf(f.file, (err) => {
              if (err) {
